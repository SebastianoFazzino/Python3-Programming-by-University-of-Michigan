#given the following json file, let's say we want to analyze it and extract some data from it 
...
#let's start printing some of the characteristics of the file:

import json

#let's print the first 100 characters of the file
print(json.dumps(res, indent = 2,)[:100])

#we can check what kind of file we're dealing with
print(type(res))

#we can print out the dictionary keys
print(res.keys())

#we now now that the file has two keys, 'search_metadata' and 'statuses'
#let's assign the key 'statuses' to the variable res2
res2 = res['statuses']

print("\n---- LEVEL2 ----\n")
print(type(res2)) #res2 is a list of dictionaries!
print(len(res2))  #and it has three element in it

#let's go deeper in the third level of the file
for res3 in res2:
    print("\n---- LEVEL3: a tweet! ----\n")
    print(json.dumps(res3, indent = 2)[:30])
    print(type(res3)) #res3 is a dictionary
    #printing the keys, we see that  'user' is the one we're interested in
    print("\n",res3.keys())
   
    #so we assign the key 'user' to the variable res4
    res4 = res3['user']
    print("\n---- LEVEL4: the user who wrote the tweet ----\n")
    print(type(res4)) #res4 is a dictionary
    print("\n",res4.keys())
    
    #we can now print out the data we are interested in:
    print('\n',res4["screen_name"], res4["created_at"])
    

print("\n******************************************\n")
    
    
#now that we know were the data we're looking for is located in the file,
#we can easily extract it with a so much smaller and easier to read code:
